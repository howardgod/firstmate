...
1. **Resolve the profile.**
   ...
   A recorded [Claude gateway](configuration.md#claude-gateway-cliproxyapi) follows the replacement unless `--gateway` names a new value (`none` drops it) or the harness changes, and a kept gateway is validated against the replacement profile before the old agent stops, so an Anthropic model on a proxied task refuses until `--gateway none` says the move to the subscription is deliberate.
2. **Safe checkpoint.**
