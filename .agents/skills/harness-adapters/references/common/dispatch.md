...
A profile carrying `gateway: cliproxy` is a fourth concrete axis passed as `--gateway cliproxy`, honored only with `--harness claude` and a non-Anthropic model; `docs/configuration.md` "Claude gateway (CLIProxyAPI)" owns it, and the claude reference records the harness facts.
